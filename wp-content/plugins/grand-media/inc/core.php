...
class GmediaCore {
	...
	public function alert( $type = 'info', $content = '' ) {
		...
		return '<div class="alert alert-' . esc_attr( $type ) . ' alert-dismissable"><button type="button" class="btn-close float-end m-0" data-bs-dismiss="alert" aria-hidden="true" aria-label="Close"></button>' . esc_html( $content ) . '</div>';
	}
	...
	public function gmedia_custom_meta_box( $gmedia_id, $meta_type = 'gmedia' ) {
		...
		<fieldset id="gmediacustomstuff_<?php echo (int) $gmedia_id; ?>" class="gmediacustomstuff" data-metatype="<?php echo esc_attr( $meta_type ); ?>">
			...
			<a href="#newCustomFieldModal" data-bs-toggle="modal" data-gmid="<?php echo absint( $gmedia_id ); ?>" class="badge badge-info btn btn-outline-secondary label label-primary newcustomfield-modal"><?php esc_html_e( 'Add New Custom Field', 'grand-media' ); ?></a>
		</fieldset>
